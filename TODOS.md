...
### F-011: 「判定不能」カードの空セクションが余白を取る
**What:** signal=unknown（脂肪酸データ全食材ゼロ）のカードでも他のカードと同じ
高さで描画される。「脂肪酸の内訳」セクションが空（"該当する脂肪酸データが
ありません"）でも視覚スロットを保持しているため、3 列グリッドの 1 つだけ
スカスカに見える。
**Why:** 視覚的アンバランス。判定不能カードは情報量が少ないので、コンパクトな
状態（小さめ高さ + 説明文を 1 行）に collapse した方が、他カードとの対比で
"missing data" が際立ち、ユーザーにとっても認知負荷が下がる。
**Pros:** グリッドの視覚バランス改善、ユーザーが「なぜこれだけ違うのか？」を
直感で理解できる。
**Cons:** 異なる高さのカードを grid に並べると CSS が少し複雑（`grid-auto-rows: min-content` か `align-items: start` で対応可能）。
**Effort:** 30 分。
**Trigger:** 判定不能カードが頻出するようになったとき、または次回 design pass で。
**Context:** 2026-05-04 本番動作確認時に検出（PR #22 マージ後）。実機スクショで
3 つ目のカードが空っぽのまま他と同高さで並んでいるのを確認。

---

### F-012: 個別食事カードに、アップロードした画像のサムネイルを表示
**What:** 個別食事結果カード（MealResultCard）の上部に、その食事の元写真の
サムネイルを表示する。現状はアップロードフェーズではサムネが見えるが、結果
ページに遷移すると写真とスコアの紐付けが視覚的に切れる。
**Why:**
1. **フィードバック精度向上**: 「正確 ✓ / 誤り - 修正」ボタンを押すユーザーは、
   どの写真の判定なのかを目視で確認したい。Vision の食材識別が合っているかは、
   写真と識別結果リストを並べて見ないと判断できない。今は記憶頼り。
2. **ユーザー体験**: 「朝食 1/3」だけだと「どの朝食？」を覚えていないと混乱
   （複数日まとめ入れの場合は特に）。サムネがあれば一瞥で把握。
3. **認知負荷低減**: ユーザーが結果を共有・スクリーンショット保存するときも
   「自分の食事」感が出てエンゲージメント向上。
**Pros:**
- フィードバックの質が上がる（誤識別を発見しやすくなる）→ Vision の改善ループに効く
- 結果ページが「自分のもの」感がぐっと上がる
- 既にアップロードフェーズで Blob を持っているので、URL.createObjectURL で
  簡単に表示可能（追加データ転送なし）
**Cons:**
- カード高さが増える（モバイルでスクロール量増）
- メモリ: object URL の cleanup が必要（`URL.revokeObjectURL` を unmount 時に）
- HEIC 等を browser がデコードできない場合のフォールバック必要（ただし現状
  HEIC は分析前にエラー return なので問題なし）
**Effort:** 1-2 時間。
1. analyze API レスポンス → ResultPanel に Blob/URL を渡す経路を追加
2. MealResultCard 上部に `<img>` をレスポンシブ表示（max-h-32 等）
3. `URL.revokeObjectURL` の cleanup（useEffect 戻り値）
4. レイアウト調整（写真上、見出し中、データ下）

**Trigger:** 次のフィードバック改善 / コーチング体験向上 PR と一緒に。
**Context:** 2026-05-04 本番動作確認時のユーザー要望から。F-011 と同セッション。
