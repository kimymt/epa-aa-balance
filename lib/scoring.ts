// 脂質ベース EPA/AA バランス計算 (v0.3.0+)
//
// v0.2.0 では「魚タンパク質 / 総タンパク質」を proxy として使っていたが、
// EPA / AA / DHA は脂肪酸（脂質）であって proxy では正確でないため、
 ...
 * - "unknown": データ不足で判定不能 (lipidPct=null のケース)
 */
...
export function computeLipidScore(foods: VisionFood[]): LipidResult {
  ...
  if (matched.length === 0) {
    return {
      lipidPct: null, lipidRatio: null, signal: "unknown",
      epaMg: 0, dhaMg: 0, aaMg: 0,
      lipidCoverage, matched, excludedNoData, unmatched,
    };
  }
